...
test("movie detail and cards render spoken languages as accessible flags", async () => {
  ...
  assert.match(renderer, /const alternativeTitle = alternativeMovieTitle\(movie\)[\s\S]*movie\.releaseYear[\s\S]*spokenLanguagesElement\(movie, screenings, \{ showCodeFallback: true \}\)/u);
  assert.match(renderer, /titleRow\.className = "dialog-original-title"[\s\S]*detailsRow\.className = "dialog-movie-details"/u);
  assert.match(source, /function spokenLanguagesElement\(movie, screenings = \[\], \{ showCodeFallback = false \} = \{\}\)/u);
  ...
  assert.match(source, /if \(!flagPath && !showCodeFallback\) continue/u);
  assert.match(source, /image\.addEventListener\("error"[\s\S]*if \(showCodeFallback\)[\s\S]*else language\.remove\(\)/u);
